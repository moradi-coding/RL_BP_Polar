...
int main() { 
	...
	long i,a,k,i2,j,l,len_vndeg,len_cndeg,iter,iter_max=120,flg,base,zeta_indx,opt=0,K,num,m=134,**zeta,num_zeta=pow(10,4),fn,snr_mix=0;
	...
	**rwrd,sum_j1_to_i,epsilon=0.6,beta=0.9,beta_i,*sum_rwd,*V,**Q, nu,
	...
	long vndeg1[]={1,2,3}, cndeg1[]={4}; //target rate [7,4] Hamming
	float lambda1[]={0.429,0.429,0.142}, rho1[]={1}; 
	
	long vndeg2[]={2}, cndeg2[]={7}; float lambda2[]={1}, rho2[]={1}; fn=8; R=0.714; nu=1; //(2,7) base LDPC code
	//long vndeg2[]={2,0,0}, cndeg2[]={15}; float lambda2[]={1,0,0}, rho2[]={1}; fn=11; R=0.867; nu=1; //(2,15) base LDPC code
	...
	for(i2=0;i2<num;i2++) {
		...
		for(iter=0; iter<iter_max; iter++) { //no. of decoder iters.
			for(a=0;a<m;a++) { //a is index of scheduled CN
				...
				while(zeta_indx<num_zeta) { //randomly generate num_zeta vectors for each VN degree (have to be random since zeta vec depends on a)
					for(i=0;i<len_vndeg;i++) 
					...
					for(i=0;i<len_vndeg;i++) {	
						if(zeta[i][a]) {
							sum1=sum2=0;
							for(j=0;j<a;j++)
								sum1+=zeta[i][j]*mu_cn_vn[j]; //CN to VN mean of jth CN
							for(j=a+1;j<m;j++)
								sum2+=zeta[i][j]*mu_cn_vn[j];
							//mu_vn_cn[i]=mu_0+(vndeg[i]-1)*mu_cn_vn; //mean of VN to CN message	
							if(!snr_mix) {
								mu_vn_cn[i][a][zeta_indx]=mu_0+sum1+(zeta[i][a]-1)*mu_cn_vn[a]+sum2;
								phi_mu_vn_cn[i][zeta_indx]=exp(-0.4527*pow(mu_vn_cn[i][a][zeta_indx],0.86)+0.0218); 
							}
							else { 
								phi_mu_vn_cn_sum_k[i][zeta_indx]=0;                                
								for(l=0;l<K;l++) {
									mu_vn_cn_mix[i][a][zeta_indx][l]=mu_0_vec[l]+sum1+(zeta[i][a]-1)*mu_cn_vn[a]+sum2;
		...
		for(a=0;a<m;a++) {
			for(long lmax=0;lmax<iter_max;lmax++) {
				sum2=0; 
				for(i=1;i<lmax;i++) {
					sum_j1_to_i=0;
					for(j=1;j<=i;j++) 
